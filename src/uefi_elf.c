...
elf_file_t uefi_elf_load_from_path(const char* path) {
	FILE *f;
	char *buff = NULL;
	long int size = 0;
	elf_file_t ret_val = {size, buff};
	...
	ret_val.filesize = size;
	ret_val.buffer = buff;
	return ret_val;
}

void uefi_elf_load_and_jump(const elf_file_t* file, const bootparams_t* bp)
{
	if(!file->filesize || !file->buffer){
		fprintf(stderr, "ELF load and jump passed invalid file handle\n");
		return;
	}
	Elf64_Ehdr *elf;
	Elf64_Phdr *phdr;
	...
	if(!memcmp(elf->e_ident, ELFMAG, SELFMAG) &&	/* magic match? */
		...
		elf->e_phnum > 0) {						 /* has program headers? */
			/* load segments */
			for(phdr = (Elf64_Phdr *)(file->buffer + elf->e_phoff), i = 0;
				i < elf->e_phnum;
				i++, phdr = (Elf64_Phdr *)((uint8_t *)phdr + elf->e_phentsize)) {
					if(phdr->p_type == PT_LOAD) {
						printf("ELF segment %p %d bytes (bss %d bytes)\n", phdr->p_vaddr, phdr->p_filesz,
							phdr->p_memsz - phdr->p_filesz);
						memcpy((void*)phdr->p_vaddr, file->buffer + phdr->p_offset, phdr->p_filesz);
	...
	i = (*((int(* __attribute__((sysv_abi)))(const bootparams_t*))(entry)))(bp);
